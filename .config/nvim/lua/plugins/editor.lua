return {
...

  {
    ...
    init = function()
      -- The main branch ships its queries/parsers under `runtime/`, which Neovim
      -- does not pick up automatically (only the plugin root lands on the rtp).
      -- Without this, no language resolves highlight/injection queries.
      local root = require("lazy.core.config").options.root
      vim.opt.runtimepath:prepend(root .. "/nvim-treesitter/runtime")
    end,
  },
